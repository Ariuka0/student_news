...
const DEMO_POSTS = [
  {
    ...
    userId: "demo-user-1", authorName: "Амина",
    createdAt: new Date(Date.now() - 1000 * 60 * 25).toISOString(),
    updatedAt: new Date(Date.now() - 1000 * 60 * 25).toISOString(),
    likesCount: 18, likedByMe: false, isOwner: false, image_url: null,
...
// ── FIX 1: Supabase client with session detection options ──
function configureSupabase() {
  const config    = window.APP_CONFIG ?? {};
  const hasClient = Boolean(window.supabase?.createClient);
  const hasUrl    = Boolean(config.SUPABASE_URL);
  // Validate that the key looks like a real JWT (starts with eyJ), not a shorthand key
  const hasKey    = Boolean(config.SUPABASE_ANON_KEY && config.SUPABASE_ANON_KEY.startsWith("eyJ"));
 
  state.isConfigured = hasClient && hasUrl && hasKey;
...
function bindEvents() {
  ...
  uploadArea.addEventListener("keydown", (e) => {
    if ((e.key === "Enter" || e.key === " ") && !uploadArea.classList.contains("disabled")) {
      e.preventDefault();
      el.imageFileInput().click();
    }
  });
  uploadArea.addEventListener("dragover",  (e) => { e.preventDefault(); if (!uploadArea.classList.contains("disabled")) uploadArea.classList.add("drag-over"); });
...
// ── FIX 2: Image path uses uid/ sub-folder so delete policy works ──
async function uploadImageToSupabase(file) {
  if (!state.supabase) throw new Error("Supabase not configured");
 
  const uid      = state.session.user.id;
  const ext      = file.name.split(".").pop().toLowerCase() || "jpg";
  // Path: <uid>/<timestamp>.<ext>  → storage.foldername(name)[1] returns uid
  ...
  const { data: urlData } = state.supabase.storage.from(bucket).getPublicUrl(path);
...
async function bootstrap() {
  if (!state.isConfigured) {
    ...
    render();
    showMessage("Demo preview горимд байна. config.js-д зөв JWT anon key тавьж, SQL setup хийгээрэй.", "info", true);
    return;
  }
 
  // Clean up email-confirmation token from URL after Supabase exchanges it
  ...
  try {
    ...
    state.posts = cloneData(DEMO_POSTS);
    render();
    showMessage("Supabase холболтод алдаа гарлаа. Demo горимд шилжлээ: " + (err.message || ""), "error", true);
  }
}
...
// ── FIX 3: ensureOwnProfile is non-fatal, handles upsert correctly ──
async function ensureOwnProfile() {
  ...
  try {
    ...
    console.warn("ensureOwnProfile failed (non-fatal):", err);
  }
}
 
// ── FIX 4: loadPosts uses flat queries then joins in JS ──
// The PostgREST embedded resource syntax requires a FK from news_posts → profiles.
// If that FK is present in the DB (see SQL), you can use the embed syntax.
// We also provide a safe fallback that fetches profiles separately.
async function loadPosts() {
  ...
  try {
    // Fetch posts, comments, likes, and profiles in parallel
    const [pr, cr, lr, pfr] = await Promise.all([
      state.supabase
        .from("news_posts")
    ...
    // Profiles errors are non-fatal — we fall back to email-based names
    if (pfr.error) console.warn("Profiles fetch warning:", pfr.error.message);
 
    // Build a map of user_id → display_name
    const profileMap = {};
    (pfr.data ?? []).forEach(p => { profileMap[p.id] = p.display_name; });
 
    state.posts = normalizeRemotePosts(pr.data ?? [], cr.data ?? [], lr.data ?? [], profileMap);
    return true;
 
  } catch (err) {
    console.error("loadPosts error:", err);
    showMessage("Өгөгдөл ачаалах үед алдаа гарлаа: " + (err.message || "SQL policy эсвэл config-аа шалгана уу."), "error", true);
    return false;
  } finally {
    state.loading = false;
  }
...
// ── FIX 5: normalizeRemotePosts uses the profileMap ──
function normalizeRemotePosts(posts, comments, likes, profileMap) {
  return posts.map(post => {
    const postComments = comments
      .filter(c => c.post_id === post.id)
      .map(c => ({
        id: c.id,
        authorName:  profileMap[c.user_id] || "Нэргүй хэрэглэгч",
    ...
    return {
      ...
      authorName: profileMap[post.user_id] || "Нэргүй хэрэглэгч",
...
function renderNavUser() {
  ...
  if (state.session) {
    ...
    nav.innerHTML = `<a href="./index.html" class="btn btn-primary" style="font-size:0.85rem;padding:9px 16px">Нэвтрэх</a>`;
  }
}
...
function renderComposerState() {
  ...
  if (state.demoMode) {
    el.composerHint().textContent = "Demo горимд байна. config.js-д зөв JWT anon key тавьж, SQL setup хийгээрэй.";
  } else if (locked) {
  ...
  el.imageUploadArea().classList.toggle("disabled", locked);
}
...
async function handleFeedSubmit(e) {
  ...
  if (state.demoMode) { showMessage("Comment feature-г ажиллуулахын тулд Supabase холбоно уу.", "info", true); return; }
  if (!state.session) { showMessage("Comment үлдээхийн тулд эхлээд нэвтэрнэ үү.", "error"); return; }
...
function startEdit(postId) {
  ...
  removeImage(); // Don't pre-fill old image — new upload or keep as-is
  renderComposerState();
  el.composer().scrollIntoView({ behavior: "smooth", block: "start" });
}
...
async function handleLogout() {
  ...
  if (error) { showMessage(error.message || "Гарах үед алдаа гарлаа.", "error", true); }
}
