/* app-script.js — Dashboard page
   ...
   - Supabase client created with detectSessionInUrl + persistSession options
   - uploadImageToSupabase: path uses a sub-folder so storage delete policy works
   - ensureOwnProfile: wrapped in try/catch that doesn't crash the whole bootstrap
   - bootstrap: session check waits for onAuthStateChange SIGNED_IN on fresh
     email-confirmation redirect so profile is always ready
*/
...
// ── FIX: pass auth options so detectSessionInUrl works ──
...
// ── FIX: store files under user-id sub-folder so RLS delete policy matches ──
async function uploadImageToSupabase(file) {
  if (!state.supabase) throw new Error("Supabase not configured");

  const uid      = state.session.user.id;
  const ext      = (file.name.split(".").pop() || "jpg").toLowerCase();
  const filename = `${uid}/${Date.now()}.${ext}`;   // ← sub-folder = uid
...
// ── FIX: don't throw from ensureOwnProfile so a profile table error
//         doesn't kill the whole dashboard ──
async function ensureOwnProfile() {
  ...
  try {
    ...
    state.profile = payload; // still use the derived name
  }
}
...
function normalizeRemotePosts(posts, comments, likes) {
  return posts.map(post => {
    const postComments = comments
      ...
      .map(c => ({
        id: c.id,
        authorName:  getRelatedDisplayName(c.profiles),
...
async function toggleLike(postId) {
  ...
  try {
    if (post.likedByMe) {
      ...
      const { error } = await state.supabase.from("post_likes")
